...
export function getCharacterTile(char: string, textColor = 0x3F, bgColor = 0x30): Uint8Array {
    const charCode = char.charCodeAt(0) & 255;
    const index = charCode * 8;
    const pattern = font.slice(index, index + 8);
    const tile = new Uint8Array(64);

    for (let y = 0; y < 8; y++) {
        const row = pattern[y];
        for (let x = 0; x < 8; x++) {
            tile[y * 8 + x] = (row & (1 << (7 - x))) !== 0 ? textColor : bgColor;
        }
    }
