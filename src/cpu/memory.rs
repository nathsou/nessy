pub trait Memory {
    ...
    fn write_word(&mut self, addr: u16, val: u16) {
        ...
        self.write_byte(addr, ((val >> 8) & 0xff) as u8);
    }
}

pub trait MappedMemory {
    fn read_byte(&mut self, addr: u16, other: &mut Box<dyn Memory>) -> u8;
    fn write_byte(&mut self, addr: u16, val: u8, other: &mut Box<dyn Memory>);

    fn read_word(&mut self, addr: u16, other: &mut Box<dyn Memory>) -> u16 {
